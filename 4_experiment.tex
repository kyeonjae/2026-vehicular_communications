...
AE traces after deduplication and preprocessing. \emph{Prev.}\ is the anomalous-window fraction, which is also the no-skill AUPRC
baseline. At $N = 64$ and stride~1 one malicious packet marks up to 64 windows, so it
runs about twice the packet-level attack ratio (23.0\% overall).}
\label{tab:dataset}
\footnotesize
\begin{tabular}{lrrrr}
\toprule
\textbf{Split} & \textbf{Files} & \textbf{Windows} & \textbf{Anomalous} & \textbf{Prev.} \\
\midrule
Training (normal)           & 31 & 5,433,175 & 0 & 0\% \\
Validation (normal)         &  8 & 3,066,673 & 0 & 0\% \\
\midrule
RTP Stream Replacement      &  5 & 1,291,777 &   301,173 & 23.3\% \\
Nmap Scanning \& Spoofing   & 22 & 3,811,885 & 1,998,714 & 52.4\% \\
\textbf{Total Attack Files} & \textbf{27} & \textbf{5,103,662} & \textbf{2,299,887} & \textbf{45.1\%} \\
\bottomrule
\end{tabular}
\end{table}
