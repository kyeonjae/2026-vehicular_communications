...
The delta input is computed from the original unmasked content (Eq.~\eqref{eq:delta}) and
remains unmasked throughout, preserving the integrity of the variation signal. Likewise, the
IPI sequence $\boldsymbol{\iota}$ (Eq.~\eqref{eq:ipi}) is left unmasked at every position,
...
\begin{equation}
    ...
    \label{eq:posenc}
\end{equation}
Here, $\text{Dropout}(\cdot)$ is applied at rate $p_{\text{drop}} = 0.1$, and $\mathbf{W}_c \in \mathbb{R}^{L \times d_{\text{model}}}$ is the projection weight. This same rate is used uniformly across the network: in Eq.~\eqref{eq:posenc} above, inside each Transformer encoder layer of Eq.~\eqref{eq:transformer}, and in the latent refinement step of Eq.~\eqref{eq:latent}. We consider two schemes for instantiating $\mathbf{P}_{\text{enc}}$, compared directly in the ablation study of Section~\ref{sec:ablation}.
