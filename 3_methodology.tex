...
\begin{figure*}[pos=t]
\centering
\includegraphics[width=.8\textwidth]{figs/figure-proposed_ids_architecture.pdf}
\caption{Deployment position and threat context for the proposed AE-IDS. The ADAS, chassis and safety, electric powertrain, and infotainment/connectivity domains exchange inter-domain traffic through the Automotive Ethernet backbone and central gateway, where the detector passively observes raw AE packets and their inter-packet byte differences. Markers illustrate the range of attacks possible against Automotive Ethernet, spanning three broad classes: (A) network access and reconnaissance, (B) application-layer exploitation, and (C) network-layer spoofing. Of these, Nmap-based scanning/spoofing (A) and RTP stream replacement (B) are evaluated in this work (Section~\ref{sec:experimental_setup}).}
\label{fig:ae_deployment}
\end{figure*}

Figure~\ref{fig:ae_deployment} situates the proposed method: it is deployed at the central gateway, which aggregates inter-domain Automotive Ethernet traffic from a representative functional partition of a modern electric vehicle and offers a single, passive vantage point (e.g., via port mirroring) without participating in packet forwarding. This centralized position motivates a protocol-agnostic design that processes both raw packet bytes and their inter-packet differences, since the attack classes it marks manifest as changes to headers, addressing, or payload content that neither signal alone fully captures.

We formulate Automotive Ethernet intrusion detection as an unsupervised anomaly detection problem. Let a raw packet be represented as a byte sequence $\mathbf{p}_i \in \{0, \ldots, 255\}^{L}$, where $L = 128$ is the fixed per-packet byte length: shorter packets are zero-padded, and longer ones are truncated to their leading $L$ bytes. Section~\ref{sec:bytelen_ablation} examines the effect of this choice. Each byte value is normalized to $[0, 1]$ by dividing by 255. A traffic window is defined as a temporally ordered sequence of $N = 64$ consecutive packets, $\mathbf{X} = [\mathbf{p}_1, \mathbf{p}_2, \ldots, \mathbf{p}_N] \in \mathbb{R}^{N \times L}$, constructed using a sliding window of stride 1 within each capture trace, so that no window spans a trace boundary.
...
$a(\cdot)$                      & attention-pooling scoring function \\
$s(\mathbf{X})$                 & window anomaly score (mean reconstruction error) \\
...
\begin{equation}
    ...
    \label{eq:ipi}
\end{equation}
mirroring the zero-initialization convention used for the delta sequence (Eq.~\ref{eq:delta}). The logarithmic compression $\log(1+|\cdot|)$ accounts for the wide dynamic range of Automotive Ethernet inter-arrival times, which span several orders of magnitude (microseconds to seconds) depending on traffic type and driving scenario; without this compression, occasional large inter-arrival gaps would dominate the learned representation. The IPI sequence is derived solely from packet capture timestamps and is independent of any upper-layer protocol parsing, preserving the protocol-agnostic nature of the proposed method.

\noindent\textbf{Masking.}
Prior to encoding, a packet-level binary mask $\mathbf{m} \in \{0, 1\}^{N}$ is sampled to 
determine which packets within the window are masked: each position $i \in \{1, \ldots, N\}$ 
is independently drawn as $m_i \sim \text{Bernoulli}(p_{\text{mask}})$ with $p_{\text{mask}} 
= 0.20$, so that approximately 20\% of the $N$ packets in each window are selected for 
masking. For each position where $m_i = 1$, the entire $L$-dimensional content vector 
$\mathbf{p}_i$ is replaced with a single shared, learned mask token $\mathbf{e}_{\text{mask}} 
\in \mathbb{R}^{L}$, initialized from $\mathcal{N}(0, 0.02^2)$ and updated during training 
via backpropagation:
\begin{equation}
    \tilde{\mathbf{p}}_i =
    \begin{cases}
...
Masked modeling architectures broadly fall into two paradigms, distinguished by \emph{where} 
the mask token is introduced. BERT~\cite{devlin2019bert} substitutes masked positions with a 
shared learnable placeholder and passes the full corrupted sequence directly to the encoder, 
allowing self-attention to mix information across masked and unmasked positions; MAE~\cite{he2022masked} instead discards masked patches from the encoder input entirely and 
reintroduces the mask token only at the decoder, so its encoder never observes a masked 
position. We adopt the former, applying $\mathbf{e}_{\text{mask}}$ directly at the encoder 
input as in Eq.~\eqref{eq:mask}: recovering the content of a masked packet requires the 
attention mechanism to reference neighboring packets directly during encoding, which is only 
possible if the full, mask-augmented sequence remains visible to the encoder throughout. We 
further substitute every masked position through this single, consistent mechanism, without 
introducing additional corruption types such as random-token substitution, so that the 
reconstruction signal at each masked position is unambiguous and does not require the model 
to first infer which corruption was applied to a given position, which supports reliable 
reconstruction-error-based anomaly scoring at inference (Section~\ref{sec:scoring}).

The delta input is computed from the original unmasked content (Eq.~\ref{eq:delta}) and
remains unmasked throughout, preserving the integrity of the variation signal. Likewise, the
IPI sequence $\boldsymbol{\iota}$ (Eq.~\ref{eq:ipi}) is left unmasked at every position,
including where $m_i = 1$: only the content vector $\mathbf{p}_i$ is replaced by 
$\mathbf{e}_{\text{mask}}$, while the inter-packet timing used to compute 
$\mathbf{P}_{\text{enc}}$ is left intact. Masking the timing signal as well would confound the 
reconstruction objective, forcing the decoder to infer both the missing content and the 
missing timing of a packet from a single shared token.
...
\begin{equation}
    ...
    \label{eq:posenc}
\end{equation}
Here, $\text{Dropout}(\cdot)$ is applied at rate $p_{\text{drop}} = 0.1$, and $\mathbf{W}_c \in \mathbb{R}^{L \times d_{\text{model}}}$ is the projection weight. The same rate $p_{\text{drop}} = 0.1$ is used uniformly across the network: in Eq.~\eqref{eq:posenc} above, inside each Transformer encoder layer of Eq.~\eqref{eq:transformer}, and in the latent refinement step of Eq.~\eqref{eq:latent}. We consider two schemes for instantiating $\mathbf{P}_{\text{enc}}$, compared directly in the ablation study of Section~\ref{sec:ablation}.

\noindent\textit{Sinusoidal positional encoding (baseline).} Following the conventional Transformer scheme~\cite{vaswani2017attention}, $\mathbf{P}_{\text{enc}} = \mathbf{P}$ is a fixed matrix computed from each packet's discrete rank $i \in \{1, \ldots, N\}$ within the window using the standard sine/cosine basis. This scheme encodes only the \emph{ordinal position} of a packet (1st, 2nd, \ldots, 64th) and is invariant to the actual elapsed time between packets.

\noindent\textit{Time-aware positional encoding (proposed).} Automotive Ethernet attacks such as high-rate port scanning, packet replay, and flooding typically manifest as anomalous \emph{inter-arrival timing} rather than anomalous sequence rank, a signal that the sinusoidal scheme cannot represent. We therefore propose conditioning the positional signal directly on the IPI sequence $\boldsymbol{\iota}$ (Eq.~\ref{eq:ipi}) via a learnable projection:
\begin{equation}
...
\FOR{epoch $= 1$ \TO $E$}
    \FOR{each mini-batch $\{\mathbf{X}^{(k)}\}$ from training set}
        ...
        \STATE Encode content stream $\mathbf{C}$ (Eqs.~\ref{eq:posenc}--\ref{eq:transformer}, with $\mathbf{P}_{\text{enc}} = \mathbf{T}$ from Eq.~\eqref{eq:timepe}) and delta stream $\mathbf{D}$ (Eq.~\ref{eq:deltaproj})
        \STATE Fuse streams into $\mathbf{F}$ via gated residual fusion (Eqs.~\ref{eq:gate}--\ref{eq:fusion})
        \STATE Compute latent vector $\mathbf{z}$ via attention pooling and refinement (Eqs.~\ref{eq:attnpool}--\ref{eq:latent})
...
\label{sec:fusion}
The content representation $\mathbf{C}$ (Eq.~\ref{eq:transformer}) and delta representation $\mathbf{D}$ (Eq.~\ref{eq:deltaproj}) are combined through a delta-conditioned gated residual mechanism. A gate is computed from the delta representation alone and used to modulate its contribution to the content representation:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:fusion}
\end{equation}
Here, $\sigma(\cdot)$ denotes the sigmoid activation and $\odot$ denotes element-wise multiplication, with the learned weight matrix $\mathbf{W}_g \in \mathbb{R}^{d_{\text{model}} \times d_{\text{model}}}$ used only within the gate. By conditioning the gate on the delta representation itself, the mechanism suppresses the delta stream's contribution at positions where inter-packet variation is uninformative, allowing the content stream to dominate when global sequential structure is the primary discriminative signal.

This formulation is asymmetric rather than a symmetric two-stream fusion: the content representation $\mathbf{C}$ carries the window's global content structure and enters unmodulated, while the delta representation $\mathbf{D}$ is admitted as a gated correction that injects local temporal variation only where such variation is itself informative. The asymmetry is one of \emph{routing}, not of importance -- which stream dominates is attack-dependent, and the ablation of Section~\ref{sec:arch_ablation} shows the delta path to be indispensable for RTP stream replacement, where removing it drops that category's F1 from 0.7983 to 0.0384 on the shared backbone. Because $\mathbf{D}$ already enters the fusion additively as a correction term in Eq.~\eqref{eq:fusion}, its own magnitude provides a natural, parameter-efficient proxy for how much of it should be admitted at each position, without requiring a heavier gate jointly conditioned on $[\mathbf{C} \,\|\, \mathbf{D}]$. We therefore adopt this asymmetric, delta-conditioned gating in place of symmetric content--delta gating: it keeps the fusion mechanism lightweight while leaving the content stream's representation intact through the encoder.
...
\begin{equation}
    \alpha_i = \frac{\exp\!\left(a(\mathbf{F}_i)\right)}{\sum_{j=1}^{N} \exp\!\left(a(\mathbf{F}_j)\right)}, \qquad
    \bar{\mathbf{z}} = \sum_{i=1}^{N} \alpha_i \mathbf{F}_i,
    \label{eq:attnpool}
\end{equation}
where $a(\cdot): \mathbb{R}^{d_{\text{model}}} \rightarrow \mathbb{R}$ is a learned scoring function implemented as a two-layer multilayer perceptron (MLP) with a hidden dimension of $d_{\text{model}}/2 = 64$ and a Tanh nonlinearity. Attention pooling allows the model to assign non-uniform importance weights to individual packets when forming the global window representation, emphasizing packets that are most informative for characterizing normal traffic structure.
...
\label{sec:decoder}
The decoder reconstructs the original (unmasked) packet content from the fused sequence representation $\mathbf{F}$ (Eq.~\ref{eq:fusion}), conditioned on the global latent vector $\mathbf{z}$ (Eq.~\ref{eq:latent}). For each position $i$, the latent vector is broadcast and concatenated with the local sequence representation:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:threshold}
\end{equation}
A window is classified as an intrusion if $s(\mathbf{X}) > \tau$. Note that $\tau$ itself is always computed from unlabeled normal traffic alone, regardless of how $p$ is chosen; what differs is only the criterion used to select $p$ from the candidate set $p \in \{95, 97, 99, 99.5, 99.9\}$, under one of two calibration strategies: \emph{labeled calibration}, which selects $p$ to maximize F1 on labeled attack traces, or \emph{label-free calibration}, which selects $p$ to target a desired false-positive rate on normal traffic alone (e.g., $p=95$ for a 5\% FPR).

We adopt labeled calibration and select $p = 99$, the candidate that maximizes overall F1 when the resulting $\tau$ is applied to the labeled attack traces (Section~\ref{sec:threshold}). The validation split contains benign traffic only, so these are the same 27 attack traces used for final reporting rather than a disjoint calibration set; this is consistent with common practice in unsupervised anomaly detection, and Section~\ref{sec:limitations} discusses what it does and does not affect. Section~\ref{sec:threshold} additionally reports detection performance under label-free calibration, showing a modest reduction in overall F1 but a more substantial reduction in detection of the RTP stream-replacement category specifically.