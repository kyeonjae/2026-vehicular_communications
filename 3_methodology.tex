...
reconstruction-error-based anomaly scoring at inference (Sec.~\ref{sec:scoring}).
...
$\mathbf{P}_{\text{enc}}$ remains true. Masking the timing signal as well would confound the 
...
\begin{equation}
    ...
    \label{eq:threshold}
\end{equation}
A window is classified as an intrusion if $s(\mathbf{X}) > \tau$. Note that $\tau$ itself is always computed from unlabeled normal traffic alone, regardless of how $p$ is chosen; what differs is only the criterion used to select $p$ from the candidate set $p \in \{95, 97, 99, 99.5, 99.9\}$, under one of two calibration strategies: \emph{labeled calibration}, which selects $p$ to maximize F1 on labeled validation attacks, or \emph{label-free calibration}, which selects $p$ to target a desired false-positive rate on normal traffic alone (e.g., $p=95$ for a 5\% FPR).

We adopt labeled calibration and select $p = 99$ based on the precision--recall tradeoff observed when the resulting $\tau$ is applied to labeled validation attacks, consistent with common practice in unsupervised anomaly detection. Section~\ref{sec:threshold} additionally reports detection performance under label-free calibration, showing a modest reduction in overall F1 but a more substantial reduction in detection of the RTP stream-replacement category specifically.